...
void LightFace( dface_t *f, const lightchain_t *lightchain, const directlight_t **novislight, int novislights, const vec3_t faceorg )
{
	...
	if( relight ) {
		...
		size = l.numsamples * i * 3;
		if( f->lightofs < 0 || f->lightofs + size > rgblightdatasize )
			Error( "LightFace: Error while trying to relight map: invalid lightofs value, %i must be >= 0 && < %i\n", f->lightofs, rgblightdatasize );
	} else {
		...
		if( rgblightdatasize + size > MAX_MAP_LIGHTING ) {
			if( !ranout ) {
		...
		f->lightofs = rgblightdatasize;
		rgblightdatasize += size;
	}

	nmaplightdatasize = rgblightdatasize;

	out = drgblightdata + f->lightofs;
	nmap = dnmaplightdata + f->lightofs;
	...
	for( i = 0; i < MAXLIGHTMAPS && f->styles[i] != 255; i++ ) {
		for( j = 0, sample = l.sample[i]; j < l.numsamples; j++, sample++ ) {
			...
			*out++ = bound( 0, red, 255 );
			*out++ = bound( 0, green, 255 );
			*out++ = bound( 0, blue, 255 );
